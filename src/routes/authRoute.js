...
router.get('/auth/generateAccessToken', async (req, res) => {
    ...
    try {
        ...
        const dashboardUrl = `http://localhost:5173/admin?access_token=${tokenData.access_token}&refresh_token=${tokenData.refresh_token}&expires_in=${tokenData.expires_in}&token_type=${tokenData.token_type}&scope=${encodeURIComponent(tokenData.scope)}`;
